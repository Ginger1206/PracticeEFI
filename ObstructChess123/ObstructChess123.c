...
extern "C" {
  EFI_APPLICATION_ENTRY_POINT (ObstructChess123);
}
  ...
  )
{
  ...
  */
  // Chess board setting - starts
  {
    ...
    CheckBlockInterval = (CHESS_BOARD_WIDTH - 1) / (CheckBlockOneSideOnChessBoard - 1) / 2 ;  //不除2就無法畫圖
  ...
  CheckBlockOneSideOnChessBoard = 5; //For test 210128G
  CheckBlockInterval = 60 ;//(481-1)/(5-1)/2=60 For test 210128G
  ChessBlockWidth = ChessBlockHeight = CheckBlockInterval; //For test 210128G
  */
...
//自己來
  ...
  ChessBoardX2  = ChessBoardX1 + CHESS_BOARD_WIDTH - 1;
  ChessBoardY2  = ChessBoardY1 + CHESS_BOARD_HEIGHT - 1;
  //ChessBoardLUX = ChessBoardX1 + ((CHESS_BOARD_WIDTH - 1) / 4); //X2
  ChessBoardLUY = ChessBoardY1;
  //ChessBoardRUX = ChessBoardX2 - ((CHESS_BOARD_WIDTH - 1) / 4); //X3
  //ChessBoardRUY = ChessBoardY1;
  ChessBoardLX  = ChessBoardX1;
  //ChessBoardLY  = ChessBoardY1 + ((CHESS_BOARD_WIDTH - 1) / 2); //Y0.5
  ChessBoardRX  = ChessBoardX2;
  //ChessBoardRY  = ChessBoardY1 + ((CHESS_BOARD_WIDTH - 1) / 2); //Y0.5
  //ChessBoardLBX = ChessBoardLUX;
  ChessBoardLBY = ChessBoardY2;
  //ChessBoardRBX = ChessBoardRUX;
  //ChessBoardRBY = ChessBoardY2;
  
  DrawLine (*pGraph, DesktopImage, ChessBoardLX, ChessBoardLUY, ChessBoardLX, ChessBoardLBY, SOLAR_BLACK, FullLine); 
  DrawLine (*pGraph, DesktopImage, ChessBoardLX, ChessBoardLBY, ChessBoardRX, ChessBoardLBY, SOLAR_BLACK, FullLine);
  DrawLine (*pGraph, DesktopImage, ChessBoardRX, ChessBoardLBY, ChessBoardLX, ChessBoardLBY, SOLAR_BLACK, FullLine);
  //DrawLine (*pGraph, DesktopImage, ChessBoardRBX, ChessBoardRBY, ChessBoardLBX, ChessBoardLBY, SOLAR_BLACK, FullLine);
  //DrawLine (*pGraph, DesktopImage, ChessBoardLBX, ChessBoardLBY, ChessBoardLX, ChessBoardLY, SOLAR_BLACK, FullLine);
  DrawLine (*pGraph, DesktopImage, ChessBoardLX, ChessBoardLBY, ChessBoardLX, ChessBoardLUY, SOLAR_BLACK, FullLine);
...
*/
  ...
  { // Block Start - Set position of the chess board block.
    //UINT16 StartX   = ChessBoardLX; 
	UINT16 StartX   = ChessBoardLUX;
    //UINT16 EndX     = ChessBoardRX; 
  ...
  )
{
  ...
  Status = (EFI_STATUS) InitiateGame (&pGraph, &DesktopImage, &ChessBlock); //G
  if (EFI_ERROR (Status)) {
    goto ProgramDone;
  }*/
  InitiateGame (&pGraph, &DesktopImage, &ChessBlock);

  // Start & Run Game
  //Status = (EFI_STATUS) Round (pGraph, DesktopImage, ChessBlock, isSinglePlayer);
  //Status = (EFI_STATUS) Round (pGraph, DesktopImage, ChessBlock); //G
  //if (EFI_ERROR (Status)) {
  //  goto ProgramDone;
  //}
  Round (pGraph, DesktopImage, ChessBlock);
